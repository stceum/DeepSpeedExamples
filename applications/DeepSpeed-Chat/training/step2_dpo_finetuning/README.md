...
As the paper says, "Your Language Model is Secretly a Reward Model", therefore the training arguments and the training process of DPO are mostly the same as the reward model as show in [step2 "Reward Model (RM) finetuning"](../step2_reward_model_finetuning/README.md). After the training of DPO, you will get a model that has been aligned with human preferences.
...
As DPO is treating the language model as the reward model, the dataset of DPO is in the same format with the reward model finetuning, whose item includes one "chosen" and one "rejected" output for the same input. 